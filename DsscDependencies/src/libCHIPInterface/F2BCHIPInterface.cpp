...
#define log_id() "F2BCHIPIF"

#define SuS_LOG_STREAM(type,id,output)\
  std::cout <<"-- ["#type"  ] [F2BCHIPIF  ] " << output << std::endl; std::cout.flush();

std::string CHIPInterface::chipName = "DSSC_F2B";

F2BCHIPInterface::F2BCHIPInterface(CHIPFullConfig * fullConfig)
 : CHIPInterface()
{
...
F2BCHIPInterface::F2BCHIPInterface()
: CHIPInterface()
{
}


bool F2BCHIPInterface::checkF2Registers()
{
  if(!jtagRegisters->signalNameExists("Global Control Register","PxInj_UseBG_NotHD_DAC")){
    cerr << "F2BCHIPInterface: ERROR no F2B configuration found for jtag registers" << endl;
    return false;
  }
...
bool F2BCHIPInterface::checkRegisterTypes()
{
  if(!jtagRegisters->signalNameExists("Global Control Register","PxInj_UseBG_NotHD_DAC")){
    cerr << "F2BCHIPInterface: ERROR no F2B configuration found for jtag registers" << endl;
    return false;
...
void F2BCHIPInterface::disableInjectionBits()
{
...
void F2BCHIPInterface::enableInjection(bool en, const std::string & pixelStr, bool program)
{
...
void F2BCHIPInterface::setInjectionDAC(int setting)
{
...
void F2BCHIPInterface::setInjectionMode(InjectionMode mode)
{
  m_injModeNeedsMonBus = true;
  injectionMode = mode;
...
void F2BCHIPInterface::setExtLatchMode()
{
    setInjectionMode(InjectionMode::EXT_LATCH);
}


void F2BCHIPInterface::setIntDACMode()
{
  setInjectionMode(InjectionMode::ADC_INJ);
}


void F2BCHIPInterface::setNormalMode()
{
  setInjectionMode(InjectionMode::NORM_D0);
}


void F2BCHIPInterface::setPixelInjectionMode()
{
  setInjectionMode(InjectionMode::CHARGE_BUSINJ);
}


void F2BCHIPInterface::setPowerDownBits(const std::string & pixelStr, bool powerDown)
{
...
void F2BCHIPInterface::enableMonBusCols(const std::vector<int> & cols)
{
...
// col0-5 or 0-5
void F2BCHIPInterface::enableMonBusCols(std::string colsStr)
{
...
void F2BCHIPInterface::disableMonBusCols()
{
  enableMonBusForPixels(std::vector<int>());
}


void F2BCHIPInterface::enableMonBusForPixels(const std::vector<int> & pixels)
{
  if(!m_injModeNeedsMonBus){
...
const std::vector<uint32_t> F2BCHIPInterface::getPoweredPixelsFromConfig()
{
  // TODO
...
bool F2BCHIPInterface::calibrateCurrCompDAC(bool log, int singlePx, int startSetting, int defaultValue)
{
...
void F2BCHIPInterface::setPixelInjectionGain(const std::string & pixelStr,PXINJGAIN injGain, bool program)
{
...
int F2BCHIPInterface::getInjectionDACsetting()
{
  switch (injectionMode) {
    case CHARGE_PXINJ_SUSDAC:
    case CURRENT_SUSDAC :
...
void F2BCHIPInterface::setGlobalDecCapSetting(DECCAPSETTING newSetting)
{
