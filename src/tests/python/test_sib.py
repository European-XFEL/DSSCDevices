...
@pytest.mark.parametrize(
    ...
    [
       ...
       # Firmware v1, v2
       ('(0)[0]-5.0:-4.2;(1)[0]-65535:65535;(16)[0]-5.0:-4.2;',
        {'asic00.status': 0, 'asic00.t0': -5.0, 'asic00.t1': -4.2,
         'asic01.status': 0, 'asic01.t0': float('nan'), 'asic01.t1': float('nan'),
         'asic16.status': 0, 'asic16.t0': -5.0, 'asic16.t1': -4.2,}
       ),
       ...
       ('SIB[0]|PPFC[0]|PPFC_T1:17.5[0]|PPFC_T2:65535.0[3]|PPFC_T3:65535.0[3]|PPFC_T4:65535.0[3]',
        {'ppfcT1': 17.5, 'ppfcT1Status': 0, 'ppfcT2': float('nan'), 'ppfcT2Status': 3,
        'ppfcT3': float('nan'), 'ppfcT3Status': 3, 'ppfcT4': float('nan'), 'ppfcT4Status': 3}),
...
def test_process_data(line, expected):
    result = DsscSIB.process_data(line)
    for key in expected.keys():
        assert key in result
        if isinstance(expected[key], float) and isnan(expected[key]):
