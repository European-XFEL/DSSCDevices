...
namespace karabo {
    ...
    KARABO_REGISTER_FOR_CONFIGURATION(Device, DsscProcessor)
    ...
    void DsscProcessor::expectedParameters(Schema& expected) {
        ...
        STRING_ELEMENT(expected).key("sramBlacklistFileName")
        ...
        STRING_ELEMENT(expected).key("sramCorrectionFileName")
    ...
    DsscProcessor::DsscProcessor(const karabo::data::Hash& config)
        : Device(config),
    ...
    void DsscProcessor::preReconfigure(karabo::data::Hash& incomingReconfiguration) {
        vector<string> paths;
    ...
    void DsscProcessor::changeDeviceState(const data::State & newState) {
        try {
	...
	setInputFormat(m_inputFormat);
        ...
        boost::replace_all(m_sourceId, "{}", karabo::data::toString(sourceInfix));
    ...
    void DsscProcessor::onData(const karabo::data::Hash& data,
                               const karabo::xms::InputChannel::MetaData& meta) //find data for data source
    {
       ...
       const auto imageData = data.get<data::NDArray>("image.data");
       const auto imageDataShape = data.get<vector<unsigned long long>>("image.data.shape");
       
       auto reshapedImageData = data::NDArray(data::Dims(imageDataShape[2], imageDataShape[0], imageDataShape[1]), Types::UINT16);
       ...
       if (m_trainMonitorStart) {
           m_startTrainId = meta.getTimestamp().getTid();
           m_trainMonitorStart = false;
       }
       
       m_endTrainId = meta.getTimestamp().getTid();
       m_receivedTrains++;

       if (m_preview) updatePreviewData(data, m_endTrainId);
       ...
       processTrain(data.get<data::NDArray>("image.data"),
                    data.get<data::NDArray>("image.cellId"),
                    meta.getTimestamp().getTid());

    }


    void DsscProcessor::processTrain(const karabo::data::NDArray& data, const karabo::data::NDArray& cellId, unsigned long long trainId) {
        // standard c++ data structures to use for access in data
	cout << trainId << " processTrain:" << endl;
	...
	cout << "                " << m_numFrames << endl;
        //do not copy data
        NDArray asicNDArray(pixel_wise_data_ptr, utils::s_totalNumWords, data::NDArray::NullDeleter(), Dims(utils::s_numAsics, utils::s_numAsicPixels, utils::s_numSram));
	
        //NDArray asicNDArray(pixel_wise_data_ptr, utils::s_totalNumWords, data::NDArray::NullDeleter(), Dims(utils::s_numAsics, utils::s_numAsicPixels, numSram));


        data::Hash pixelDataHash;
...
#pragma omp parallel for
    ...
    void DsscProcessor::displayPixelCells(const karabo::data::Hash& data) {
        ...
        auto dataPtr = data.get<data::NDArray>("image.data").getData<uint16_t>();
    ...
    void DsscProcessor::updatePreviewData(const karabo::data::Hash& data, unsigned long long trainId) {
        //

        auto imageData = data.get<data::NDArray>("image.data");
        auto cellData = data.get<data::NDArray>("image.cellId");
        ...
        if (m_previewMaximum) {
            //
            unsigned long long previewMaxFract = trainId % m_previewMaxCounter;
