...
USING_KARABO_NAMESPACES
    ...
    const std::unordered_map<std::string, std::string> DsscDataReceiver::m_directoryStructure{
    ...
    void DsscDataReceiver::saveCurrentPixelHisto() {
        ...
        const auto dataHisto = get<bool>("showOnlineHistogram") ? m_pixelHistoVec[selectedPixel] : m_pixelHisto;
    ...
    util::Hash DsscDataReceiver::getMonitorData() {
        const auto & cellIds = m_currentTrainData->cellIds;

        NDArray imageData(m_currentTrainData->imageData(), m_currentTrainData->getNumData(), NDArray::NullDeleter(), getDims());
        ...
        monitorData.set<string>("imageFormat", m_currentTrainData->getFormatStr());
    ...
    util::Dims DsscDataReceiver::getDims() {
        util::Dims dims;
        if (m_currentTrainData->getFormat() == utils::DsscTrainData::DATAFORMAT::PIXEL) {
            dims = Dims(m_currentTrainData->availableASICs.size(), utils::s_numAsicPixels, m_currentTrainData->pulseCnt);
        } else if (m_currentTrainData->getFormat() == utils::DsscTrainData::DATAFORMAT::ASIC) {
            dims = Dims(m_currentTrainData->pulseCnt, m_currentTrainData->availableASICs.size(), utils::s_numAsicPixels);
        } else if (m_currentTrainData->getFormat() == utils::DsscTrainData::DATAFORMAT::IMAGE) {
            dims = Dims(m_currentTrainData->pulseCnt, 128, 512);
        }
        return dims;
    }
    ...
    util::Hash DsscDataReceiver::getImageData(int frameNum, int asic) {
        ...
        if (asicIdx >= 0) {
            ...
            if (m_showThreshold) {
                ...
                m_asicHisto.addToBuf(s_imageData.begin(), s_imageData.end());
            }
        }
    ...
    util::Hash DsscDataReceiver::getPixelData(int asicPixel, int asic) {
        ...
        if (asicIdx < 0) {
            ...
            if (get<bool>("correction.showCorrect") && m_bgDataValid) {
                ...
                m_pixelHisto.addToBuf(m_pixelData.begin(), m_pixelData.end());
            }
        }
...
#pragma omp parallel for num_threads(nthreads)
    ...
    void DsscDataReceiver::updateCorrectionMap() {
        ...
        const auto asicsToSort = m_currentTrainData->availableASICs;
        const int numAsics = asicsToSort.size();
        const uint pulseCnt = m_currentTrainData->pulseCnt;
...
#pragma omp parallel for num_threads(nthreads)
        for (int asicIdx = 0; asicIdx < numAsics; asicIdx++) {
            ...
            for (uint32_t px = 0; px < utils::s_numAsicPixels; px++) {
                ...
                auto pixelDataArr = m_currentTrainData->getPixelDataPixelWise(imagePixel);
...
#pragma omp parallel for
    ...
    std::string DsscDataReceiver::getDir(const std::string & name) {
        const auto it = m_directoryStructure.find(name);
        if (it != m_directoryStructure.cend()) {
            return it->second;
        }

        set("status", "unknown parameter name " + name);
    ...
    void DsscDataReceiver::updatePixelHistos() {
        ...
        m_processor.fillDataHistoVec(m_currentTrainData, m_pixelHistoVec, false);
    }
    ...
    void DsscDataReceiver::displayPixelHistogram() {
        ...
        if (get<bool>("showOnlineHistogram")) {
            ...
            m_pixelHisto.fillBufferToHistoMap();
            m_pixelHisto.getDrawValues(bins, binValues);
            histoValueCount = m_pixelHisto.getCount();
        }
    ...
    void DsscDataReceiver::displayAsicHistogram() {
        ...
        m_asicHisto.fillBufferToHistoMap();
        m_asicHisto.getDrawValues(bins, binValues);
        ...
        while (m_runDisplay) {
            ...
            if (m_trainDataToShow->isValid()) {
                ...
                if (get<bool>("saveImageWiseSorted")) {
                    m_currentTrainData = m_trainDataToShow;
    ...
    void DsscDataReceiver::receiveData() {
        ...
        try {
            do {
                ...
                m_currentTrainData = m_trainSorter.getNextTrainData();
                if (m_currentTrainData == nullptr) {
                    keepReceiving = false;
                    m_isStopped = true;
                    break;
                }
    ...
    void DsscDataReceiver::processCurrentTrainData() {
        checkTestPattern(m_currentTrainData);

        updateCurrentTrainIdAndCnt(m_currentTrainData->trainId);

        // dont send data if not all ASICs have valid test pattern (little protection)
        bool allAsicsDataValid = allASICTestPatternsValid(m_currentTrainData);
    ...
    void DsscDataReceiver::finishAcquisition() {
        ...
        if (m_genTrainStats) {
            ...
            utils::MeanRMSVectors meanRMSValuesVec = DsscHDF5Writer::saveToFile(get<string>("outputDir") + "/TrainMeanRMSImage.h5", m_dataStatsAcc, m_currentTrainData->availableASICs, numStoredTrains);

            if (m_acquireBG) {
                m_pixelBackgroundData = meanRMSValuesVec.meanValues;
                computeSramCorrectionData();

                DsscHDF5Writer::saveBaselineAndSramCorrection(get<string>("outputDir") + "/SRAMCorrectionImage.h5", m_sramCorrectionAcc, m_currentTrainData->availableASICs, numStoredTrains);
